...
def run_lr_sweep(
    ...
    for lr in LR_CANDIDATES:
        print("\n-----------------------------------")
        print(f"LR Sweep: learning_rate = {lr}")
        print("-----------------------------------")
    ...
    if len(successful_results) == 0:
        raise RuntimeError("All LR sweep runs failed. No best learning rate found.")

    best_result = min(successful_results, key=lambda x: x["final_val_loss"])
    best_lr = best_result["learning_rate"]

    results_json_path = output_dir / "lr_sweep_results.json"
    ...
    summary_csv_path = output_dir / "lr_sweep_summary.csv"
